...
void Notifier::setup() {
    ...
    _webSocket.onEvent([this](AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg,
...
void Notifier::sendit(std::string content) {
    ...
    logd("SMTP: %s:%d From:%s PW: %s To: (%s)%s", _smtpServer.c_str(), _smtpPort, _senderEmail.c_str(),
         _senderPassword.c_str(), _recipientName.c_str(), _recipientEmail.c_str());
...
message.text.transfer_encoding = Content_Transfer_Encoding::enc_7bit;*/
    ...
    if (!_smtp->isLoggedIn()) {
        ...
        if (!MailClient.sendMail(_smtp, &message)) {
            loge("SMTP Error, Status Code: %d, Error Code: %d, Reason: %s", _smtp->statusCode(), _smtp->errorCode(),
                 _smtp->errorReason());
        }
    }
}
