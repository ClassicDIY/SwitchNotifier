...
class NetDCE_Factory : public Factory {
 public:
   template <typename Module, typename... Args> static DCE_T<Module> *create(const config *cfg, Args &&...args) {
      return build_generic_DCE<Module>(cfg, std::forward<Args>(args)...);
   }

   template <typename Module, typename... Args> static std::shared_ptr<Module> create_module(const config *cfg, Args &&...args) {
...
class NetModule : public ModuleIf {
 public:
   explicit NetModule(std::shared_ptr<DTE> dte, const esp_modem_dce_config *cfg) : dte(std::move(dte)), apn(std::string(cfg->apn)) {}
   ...
   bool sync() { return dce_commands::sync(dte.get()) == command_result::OK; }

   void reset() { dce_commands::reset(dte.get()); }
...
extern "C" bool modem_start_network() { return dce->set_mode(esp_modem::modem_mode::DATA_MODE); }

extern "C" bool modem_stop_network() { return dce->set_mode(esp_modem::modem_mode::COMMAND_MODE); }
...
extern "C" bool modem_check_sync() { return dce->get_module()->sync(); }

extern "C" void modem_reset() { dce->get_module()->reset(); }

extern "C" bool modem_check_signal() { return dce->get_module()->check_signal(); }
