...
void WebLog::begin(AsyncWebServer *pwebServer) {
    ...
    _webSocket.onEvent([](AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg,
        ...
        if (type == WS_EVT_CONNECT) {
            ...
            logd("index: %" PRIu64 ", len: %" PRIu64 ", final: %" PRIu8 ", opcode: %" PRIu8 "\n", info->index,
                 info->len, info->final, info->opcode);
            String msg = "";
    ...
    pwebServer->on("/log", HTTP_GET,
                   [](AsyncWebServerRequest *request) { request->send(200, "text/html", web_serial_html); });
}
